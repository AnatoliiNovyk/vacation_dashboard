...
# Resolve database path under the repository's data directory
DATA_DIR = (Path(__file__).resolve().parent.parent / 'data')
DATA_DIR.mkdir(parents=True, exist_ok=True)
DB_PATH = DATA_DIR / 'vacations.db'
...
def get_all_employees():
    ...
    employees = conn.execute(
        'SELECT id, fio, ipn, role, manager_fio, vacation_days_per_year, remaining_vacation_days FROM staff'
    ).fetchall()
    conn.close()
...
def get_employee_by_ipn(ipn):
    ...
    employee = conn.execute(
        'SELECT id, ipn, role, fio, remaining_vacation_days FROM staff WHERE ipn = ?',
        (ipn,),
    ).fetchone()
    conn.close()
    if employee:
...
def get_employee_vacation_summary_by_ipn(ipn):
    ...
    LEFT JOIN (
        ...
        FROM vacations
        WHERE date(end_date) >= date('now', '-90 days')
...
def get_employee_details_for_edit(employee_id: int):
    ...
    employee_data = conn.execute(
        'SELECT id, fio, ipn, role, manager_fio, vacation_days_per_year, remaining_vacation_days FROM staff WHERE id = ?',
        (employee_id,),
    ).fetchone()
...
def update_employee_data_and_vacation(employee_id: int, updates: dict):
    ...
    try:
        ...
        old_staff_data = cursor.execute(
            'SELECT vacation_days_per_year, remaining_vacation_days FROM staff WHERE id = ?',
            (employee_id,),
        ).fetchone()
        ...
        if target_vacation_id and new_vacation_start_date and new_vacation_end_date:
            old_vacation = cursor.execute(
                'SELECT total_days FROM vacations WHERE id = ? AND staff_id = ?',
                (target_vacation_id, employee_id),
            ).fetchone()
            if old_vacation:
                old_total_days_for_target_vacation = old_vacation['total_days']
                new_total_days_for_target_vacation = date_utils.calculate_days(
                    new_vacation_start_date, new_vacation_end_date
                )
                ...
                cursor.execute(
                    'UPDATE vacations SET start_date = ?, end_date = ?, total_days = ? WHERE id = ?',
                    (new_vacation_start_date, new_vacation_end_date, new_total_days_for_target_vacation, target_vacation_id),
                )
                current_total_taken_or_booked_days = (
                    current_total_taken_or_booked_days
                    - old_total_days_for_target_vacation
                    + new_total_days_for_target_vacation
                )
        ...
        cursor.execute(
            ...
            (
                ...
                updates['ipn'],
                updates['role'],
                updates.get('manager_fio'),
                updates['vacation_days_per_year'],
                employee_id,
            ),
        )
